...
| <a name="input_files_output_path"></a> [files\_output\_path](#input\_files\_output\_path) | **Deprecated. Use the `zip` module and `package_path` input instead.**<br><br>Path where the lambda package will be created when using `files` or `files_dir`.<br>See `zip` `output_path` input for details. | `string` | `null` | no |
