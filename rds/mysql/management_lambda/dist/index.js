...
var isProperty = __nccwpck_require__(3988)
...
const SqlString = __nccwpck_require__(9878);

const Connection = __nccwpck_require__(5530);
const ConnectionConfig = __nccwpck_require__(737);
const parserCache = __nccwpck_require__(6482);
...
const Pool = __nccwpck_require__(2822);

exports.createPool = function(config) {
  const PoolConfig = __nccwpck_require__(3964);
  return new Pool({ config: new PoolConfig(config) });
};

exports.createPoolCluster = function(config) {
  const PoolCluster = __nccwpck_require__(4841);
...
exports.createServer = function(handler) {
  const Server = __nccwpck_require__(1426);
  const s = new Server();
  if (handler) {
...
exports.PoolConnection = __nccwpck_require__(2774);
...
exports.__defineGetter__(
  ...
  () => __nccwpck_require__(9767)/* .createConnection */ .I5
);

exports.__defineGetter__(
  'createPoolPromise',
  () => __nccwpck_require__(9767)/* .createPool */ .Kz
);

exports.__defineGetter__(
  'createPoolClusterPromise',
  () => __nccwpck_require__(9767).createPoolCluster
);

exports.__defineGetter__('Types', () => __nccwpck_require__(7532));
...
const crypto = __nccwpck_require__(6417);
const { xor, xorRotating } = __nccwpck_require__(4067);
...
const auth41 = __nccwpck_require__(4067);
...
const crypto = __nccwpck_require__(6417);
const { xor } = __nccwpck_require__(4067);
...
const Packets = __nccwpck_require__(8114);
const sha256_password = __nccwpck_require__(4584);
const caching_sha2_password = __nccwpck_require__(7663);
const mysql_native_password = __nccwpck_require__(488);
...
const Command = __nccwpck_require__(8316);
const Packets = __nccwpck_require__(8114);
...
class QueryEvent {
  constructor(packet) {
    const parseStatusVars = __nccwpck_require__(6068);
...
const Command = __nccwpck_require__(8316);
const Packets = __nccwpck_require__(8114);
const ClientHandshake = __nccwpck_require__(3404);
const CharsetToEncoding = __nccwpck_require__(8207);
...
const Command = __nccwpck_require__(8316);
const Packets = __nccwpck_require__(8114);
const ClientConstants = __nccwpck_require__(2649);
const CharsetToEncoding = __nccwpck_require__(8207);
const auth41 = __nccwpck_require__(4067);
...
class ClientHandshake extends Command {
  ...
  handshakeResult(packet, connection) {
    ...
    if (marker === 0xfe || marker === 1) {
      const authSwitch = __nccwpck_require__(8902);
      try {
        if (marker === 1) {
    ...
    if (!connection.authorized) {
      ...
      if (connection.config.compress) {
        const enableCompression = __nccwpck_require__(6517).enableCompression;
...
const Command = __nccwpck_require__(8316);
const Query = __nccwpck_require__(5314);
const Packets = __nccwpck_require__(8114);

const getBinaryParser = __nccwpck_require__(9443);
...
const ClientHandshake = __nccwpck_require__(3404);
const ServerHandshake = __nccwpck_require__(5145);
const Query = __nccwpck_require__(5314);
const Prepare = __nccwpck_require__(4100);
const CloseStatement = __nccwpck_require__(4990);
const Execute = __nccwpck_require__(3571);
const Ping = __nccwpck_require__(3206);
const RegisterSlave = __nccwpck_require__(2045);
const BinlogDump = __nccwpck_require__(2000);
const ChangeUser = __nccwpck_require__(4045);
const Quit = __nccwpck_require__(6156);
...
const Command = __nccwpck_require__(8316);
const CommandCode = __nccwpck_require__(4128);
const Packet = __nccwpck_require__(6062);
...
const Command = __nccwpck_require__(8316);
const Packets = __nccwpck_require__(8114);
const getTextParser = __nccwpck_require__(917);
const ServerStatus = __nccwpck_require__(6423);
const CharsetToEncoding = __nccwpck_require__(8207);
...
const CommandCode = __nccwpck_require__(4128);
const Errors = __nccwpck_require__(7548);
...
const PacketParser = __nccwpck_require__(2742);
...
function enableCompression(connection) {
  ...
  const seqqueue = __nccwpck_require__(1536);
...
const EventEmitter = __nccwpck_require__(8614).EventEmitter;
const Readable = __nccwpck_require__(2413).Readable;
const Queue = __nccwpck_require__(2342);
const SqlString = __nccwpck_require__(9878);
const LRU = __nccwpck_require__(7129);

const PacketParser = __nccwpck_require__(2742);
const Packets = __nccwpck_require__(8114);
const Commands = __nccwpck_require__(5482);
const ConnectionConfig = __nccwpck_require__(737);
const CharsetToEncoding = __nccwpck_require__(8207);
...
class Connection extends EventEmitter {
  ...
  promise(promiseImpl) {
    const PromiseConnection = __nccwpck_require__(9767)/* .PromiseConnection */ .WE;
  ...
  _resolveNamedPlaceholders(options) {
    ...
    if (this.config.namedPlaceholders || options.namedPlaceholders) {
      if (convertNamedPlaceholders === null) {
        convertNamedPlaceholders = __nccwpck_require__(3531)();
      }
...
const ClientConstants = __nccwpck_require__(2649);
const Charsets = __nccwpck_require__(4947);
...
class ConnectionConfig {
  ...
  static getSSLProfile(name) {
    if (!SSLProfiles) {
      SSLProfiles = __nccwpck_require__(3781);
    }
...
try {
  highlightFn = __nccwpck_require__(5935).highlight;
} catch (err) {
  highlightFn = text => {
...
const Types = __nccwpck_require__(7532);
const Packet = __nccwpck_require__(6062);
...
const Packet = __nccwpck_require__(6062);
const CommandCodes = __nccwpck_require__(4128);
...
const Packet = __nccwpck_require__(6062);
const StringParser = __nccwpck_require__(9211);
const CharsetToEncoding = __nccwpck_require__(8207);
...
const CursorType = __nccwpck_require__(2664);
const CommandCodes = __nccwpck_require__(4128);
const Types = __nccwpck_require__(7532);
const Packet = __nccwpck_require__(6062);
const CharsetToEncoding = __nccwpck_require__(8207);
...
class Handshake {
  ...
  setScrambleData(cb) {
    __nccwpck_require__(6417).randomBytes(20, (err, data) => {
      if (err) {
        cb(err);
        return;
      }
...
const AuthSwitchRequest = __nccwpck_require__(1453);
const AuthSwitchRequestMoreData = __nccwpck_require__(714);
const AuthSwitchResponse = __nccwpck_require__(2767);
const BinaryRow = __nccwpck_require__(5846);
const BinlogDump = __nccwpck_require__(2122);
const ChangeUser = __nccwpck_require__(2090);
const CloseStatement = __nccwpck_require__(918);
const ColumnDefinition = __nccwpck_require__(4470);
const Execute = __nccwpck_require__(8526);
const Handshake = __nccwpck_require__(3855);
const HandshakeResponse = __nccwpck_require__(5063);
const PrepareStatement = __nccwpck_require__(1446);
const PreparedStatementHeader = __nccwpck_require__(1401);
const Query = __nccwpck_require__(224);
const RegisterSlave = __nccwpck_require__(6197);
const ResultSetHeader = __nccwpck_require__(4797);
const SSLRequest = __nccwpck_require__(123);
const TextRow = __nccwpck_require__(926);
...
const ErrorCodeToName = __nccwpck_require__(7548);
const NativeBuffer = __nccwpck_require__(4293).Buffer;
...
const Packet = __nccwpck_require__(6062);
const ClientConstants = __nccwpck_require__(2649);
const ServerSatusFlags = __nccwpck_require__(6423);

const EncodingToCharset = __nccwpck_require__(787);
const sessionInfoTypes = __nccwpck_require__(5611);
...
const FieldFlags = __nccwpck_require__(226);
const Charsets = __nccwpck_require__(4947);
const Types = __nccwpck_require__(7532);
const helpers = __nccwpck_require__(5455);
const genFunc = __nccwpck_require__(7982);
const parserCache = __nccwpck_require__(6482);
const typeNames = [];
...
const Iconv = __nccwpck_require__(9032);
...
const process = __nccwpck_require__(1765);
const mysql = __nccwpck_require__(5828);

const EventEmitter = __nccwpck_require__(8614).EventEmitter;
const PoolConnection = __nccwpck_require__(2774);
const Queue = __nccwpck_require__(2342);
const Connection = __nccwpck_require__(5530);
...
class Pool extends EventEmitter {
  ...
  promise(promiseImpl) {
    const PromisePool = __nccwpck_require__(9767)/* .PromisePool */ .Kk;
...
const Connection = __nccwpck_require__(5828).Connection;
...
class PoolConnection extends Connection {
  ...
  promise(promiseImpl) {
    const PromisePoolConnection = __nccwpck_require__(9767)/* .PromisePoolConnection */ .Qr;
...
module.exports = __nccwpck_require__(883);
...
const core = __nccwpck_require__(5828);
const EventEmitter = __nccwpck_require__(8614).EventEmitter;
...
function createCompiler(config) {
  ...
  if (config.cache !== false && !cache) {
    cache = __nccwpck_require__(6898)({ max: ncache, dispose: EMPTY_LRU_FN });
  }
...
// A linked list to keep track of recently-used-ness
var Yallist = __nccwpck_require__(537)
...
module.exports = __nccwpck_require__(1847);
...
/* harmony export */ });
/* harmony import */ var mysql2__WEBPACK_IMPORTED_MODULE_0__ = __nccwpck_require__(5828);

async function handler({ database, queries }) {
    const connection = (0,mysql2__WEBPACK_IMPORTED_MODULE_0__.createConnection)(process.env.DATABASE_URL);
    if (database) {
    ...
    try {
        ...
        console.log(`Closing connection to the '${connection.config.database}' database`);
        connection.end();
    }
}
